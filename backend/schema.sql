...
-- =====================================================================
-- EXTENSIONES ADITIVAS v2: mesas/reservas con carrito de platos, factura
-- ligada a reserva, pagos a empleados con más detalle y compras con
-- ítems. Es aditivo: no borra ni renombra nada de lo ya existente.
-- (Antes vivía en migracion_v2.sql; se integra aquí para que una
-- instalación nueva desde este único archivo quede completa.)
-- =====================================================================

-- MESA: número visible al público + baja lógica
ALTER TABLE `mesa`
...
-- PRODUCTO_EMPLATADO: cupo diario disponible por plato (para el carrito
-- de reservas). categoria/imagen_url/estado ya se agregaron arriba.
ALTER TABLE `producto_emplatado`
  ADD COLUMN `cupo_diario` INT(11) NULL AFTER `estado`;

-- RESERVA_PRODUCTO: el "carrito de platos" de una reserva.
...
-- el módulo de finanzas necesita (número, monto, método de pago, cliente).
ALTER TABLE `factura`
...
-- DETALLE_COMPRA: también sirve como cabecera de compra a un almacén
-- (proveedor, fecha de emisión, monto total calculado por trigger a
-- partir de sus ítems), sin tocar el uso original ligado a equipamiento.
ALTER TABLE `detalle_compra`
...
-- Total consumido en una reserva (carrito de platos)
DELIMITER $$
...
BEGIN
    DECLARE v_total DECIMAL(10,2);
    SELECT COALESCE(SUM(rp.cantidad * pe.costo), 0) INTO v_total
...
-- Generar factura a partir de una reserva (transacción)
DELIMITER $$
...
<<<<<<< HEAD
-- Registrar pago a empleado (transacción + validación)
DELIMITER $$
...
BEGIN
    ...
    SELECT estado INTO v_estado_actual FROM `empleado` WHERE id_empleado = p_id_empleado FOR UPDATE;
