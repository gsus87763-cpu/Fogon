...
router.get('/empleados', verificarToken, permitirRoles('admin', 'rrhh'), async (req, res) => {
  const [filas] = await pool.query('SELECT * FROM EMPLEADO WHERE activo = 1 ORDER BY apellidos');
  res.json(filas);
});

// Interno: ingredientes (almacén / cocina)
router.get('/ingredientes', verificarToken, permitirRoles('admin', 'almacen', 'cocina'), async (req, res) => {
