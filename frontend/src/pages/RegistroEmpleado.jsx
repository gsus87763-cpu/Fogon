...
export default function RegistroEmpleado() {
  ...
  const [form, setForm] = useState({ nombre: '', apellidos: '', ci: '', telefono: '', correo: '', password: '', confirmarPassword: '' });
  ...
  function errorDe(campo) {
    ...
    if (campo === 'ci' && form.ci.trim().length < 4) return 'Ingresa un número de CI válido';
  ...
  async function enviar(e) {
    e.preventDefault();
    setError('');
    setTocado({ nombre: true, apellidos: true, ci: true, correo: true, password: true, confirmarPassword: true });

    if (form.nombre.trim().length < 2 || form.apellidos.trim().length < 2
      || form.ci.trim().length < 4 || !validarCorreo(form.correo)) {
    ...
    setEnviando(true);
    try {
      await api.post('/auth/registro-empleado', {
        nombre: form.nombre, apellidos: form.apellidos, ci: form.ci, correo: form.correo,
        telefono: form.telefono, password: form.password,
        captchaId: captcha?.captchaId, captchaRespuesta
      });
      navigate('/solicitud-personal', { state: { estadoSolicitud: 'PENDIENTE' } });
    } catch (err) {
  ...
  return (
    <div className="contenedor" style={{ paddingTop: 40, paddingBottom: 60, display: 'flex', justifyContent: 'center' }}>
      <div className="tarjeta" style={{ width: '100%', maxWidth: 480 }}>
        <span className="eyebrow">Trabaja con nosotros</span>
        <h1 style={{ fontSize: '1.6rem' }}>Registro de personal</h1>
        <p style={{ color: 'var(--texto-secundario)', fontSize: '.9rem' }}>
          Completa tus datos para postular. Tu cuenta quedará <strong>pendiente de aprobación</strong> por
          administración antes de que puedas iniciar sesión.
        </p>
        ...
        <form className="formulario" style={{ maxWidth: 'none', marginTop: 16 }} onSubmit={enviar} noValidate>
          ...
          <div className="campo">
            ...
            <input
              className={errorDe('ci') ? 'campo-invalido' : ''}
              value={form.ci}
              onChange={(e) => actualizar('ci', e.target.value)}
              onBlur={() => marcarTocado('ci')}
            />
            {errorDe('ci') && <span className="error-campo"><IconoAdvertencia /> {errorDe('ci')}</span>}
          </div>
          ...
          <p style={{ fontSize: '.9rem', textAlign: 'center' }}>
            ¿Ya tienes cuenta aprobada? <Link to="/login">Inicia sesión</Link>
          </p>
        </form>
      </div>
    </div>
